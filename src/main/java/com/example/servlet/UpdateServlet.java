...
public class UpdateServlet extends HttpServlet {
    //从<a th:href="@{/updateStudent(id=${student.id})}" ><span>修改</span></a>获取该行数据并显示在UpadeStudent页面
    //点击提交按钮后，将数据传回数据库
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try (org.apache.ibatis.session.SqlSession sqlSession = factory.openSession(true)) {
           StudentMapper studentMapper = sqlSession.getMapper(StudentMapper.class);
                studentList=studentMapper.getStudentById(id);
        }
        resp.setCharacterEncoding("UTF-8");
        resp.setContentType("text/html;charset=UTF-8");
        Context context = new Context();

        context.setVariable("upStudentlist",studentList);
        engine.clearTemplateCache();
        engine.process("updateStudent", context, resp.getWriter());
    }
